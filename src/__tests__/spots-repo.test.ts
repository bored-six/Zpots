...
import { fetchSpots, createSpot, confirmSpot, reportSpot } from "@/lib/spots-repo";
...
// confirmSpot
//
// ASSUMPTION (documented per task instructions): the contract only tells us
// confirmSpot "checks/records confirmer identity before incrementing" -- it
// does not name a table or column. Rather than guessing exact table/column
// names, this fake backend emulates a generic Postgres-style unique
...
// values (e.g. the same spot id AND the same confirmer id both appearing
// somewhere in the row, regardless of what the columns are named). This
// covers both a "select first, then insert" identity check and an
// "insert and catch the unique-violation" identity check without hardcoding
// which one the implementation picks. The main "spots" table is assumed to
// be named "spots" with an "id" column, matching the Spot type this repo
...
describe("confirmSpot", () => {
  it("does not increase confirmations when the same confirmerId confirms the same spot twice", async () => {
    const spotRow = makeSpotRow();
    const { client, db } = createFakeSupabase({ spots: [spotRow] });
    vi.mocked(getSupabaseClient).mockReturnValue(client as never);

    await confirmSpot("spot-1", "confirmer-a");
    await confirmSpot("spot-1", "confirmer-a");
  ...
  it("increases confirmations for two distinct confirmerIds on the same spot", async () => {
    const spotRow = makeSpotRow();
    const { client, db } = createFakeSupabase({ spots: [spotRow] });
    vi.mocked(getSupabaseClient).mockReturnValue(client as never);

    await confirmSpot("spot-1", "confirmer-a");
    await confirmSpot("spot-1", "confirmer-b");
  ...
  it(`flips status to 'confirmed' only once confirmations reach CONFIRMATION_THRESHOLD (currently ${CONFIRMATION_THRESHOLD}), not before`, async () => {
    ...
    for (let i = 0; i < confirmerIds.length - 1; i++) {
      await confirmSpot("spot-1", confirmerIds[i]);
      const row = db.spots.find((r) => r.id === "spot-1");
      expect(row?.status).toBe("unconfirmed");
    }

    await confirmSpot("spot-1", confirmerIds[confirmerIds.length - 1]);
    const finalRow = db.spots.find((r) => r.id === "spot-1");
  ...
  it("resolves to the updated spot reflecting the new confirmation count", async () => {
    ...
    const result = await confirmSpot("spot-1", "confirmer-a");
