...
interface SpotMapProps {
   ...
   * with why it's on the caller's map.
   */
  ...
  /** Omit to hide the Confirm control on a `mapSpots` popup. */
  onConfirmSpot?: (spotId: string) => Promise<void>;
  /** Omit to hide the Report control on a `mapSpots` popup. */
...
/**
 * The raster-fallback notice (Pergamino PRD D6): a small chip shown when the
 * vector basemap can't be read (PlaceLabelsLayer and this chip are both
 * SpotMap-only, per D3/T3.2).
 */
 ...
 *
 * paseo-motion.md fix-round-2, finding 1: the marker loop below builds
...
/** `DensityPoint`/tier lookup key for a `mapSpots` entry (pin-revamp-spec.md section 5.7). */
...
/**
 * Renders the map and its `mapSpots` markers/popups (Mi mapa). Posting a new
 * spot goes through `PostFlow`, not this component.
 */
export default function SpotMap({
  ...
  // not memoizing this).
  const densityPoints: DensityPoint[] = visibleMapSpots.map((spot) => ({
