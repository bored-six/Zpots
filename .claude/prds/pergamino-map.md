...
## Label-tuning pass: three more defects found live (post-launch, round 2)

Three more defects found by inspecting the running map at z11-z14 on desktop and 375px, none
from a new requirement, all diagnosed before implementation and fixed test-first.

| 2026-09-21 | **Defect 1 — very long protected-area names swamped the map.** New `shortenNaturalPoiName` (`pergamino-style.ts`, exported alongside a new `NATURAL_POI_NAME_MAX_CHARS = 32`) and a new `withShortenedNaturalPoiName` wrapper (`BasemapLayer.tsx`) applied only to `label-poi-natural`'s `CenteredTextSymbolizer`. Two rules, in order: (1) prefer the part of the name before a recognised protected-area qualifier phrase ("Protected Landscape", "Natural Park", "Marine Sanctuary", etc.) — `"Great and Little Santa Cruz Islands Protected Landscape & Seascape"` becomes `"Great and Little Santa Cruz Islands"`, the real name, not a truncation artifact; (2) if no qualifier matches (or it sits at the very start, leaving nothing before it) and the name still exceeds the 32-char budget, cut at the last whole word inside that budget and append a single `…`. A name already at or under budget (e.g. "Pasonanca Natural Park", 22 chars) is untouched. The wrapper never mutates the tile-cache's shared `Feature` object — it clones only when there's something to shorten, and passes the original reference through otherwise. | protomaps-leaflet's `TextAttr` only ever reads `feature.props[...]` verbatim (confirmed by reading `node_modules/protomaps-leaflet/src/attribute.ts` — `labelProps` only selects *which* prop key to read, `textTransform` only accepts the literal keywords "uppercase"/"lowercase"/"capitalize"); there is no options-level hook to transform the text a symbolizer draws, so the rewrite has to happen at the one call site between the archive's feature and the symbolizer. `maxLineChars` (protomaps-leaflet's own line-wrap option, default 15) was considered and rejected as the whole fix: it wraps a string of any length into as many lines as needed and never truncates (`node_modules/protomaps-leaflet/src/text.ts`'s `linebreak` recurses until the whole string is consumed) — it's what produced the four-plus stacked lines in the first place, not a cap on them. A max-line-count option doesn't exist in protomaps-leaflet's `TextSymbolizerOptions` to configure instead. |
| 2026-09-21 | **Defect 2 — `neighbourhood` (subdivisions) competed on equal footing with `macrohood` (real barangays) at z13.** `label-district-neighbourhood`'s `minzoom` moved from 13 to 14 (one zoom later than `label-district-macrohood`'s 11); its `CenteredTextSymbolizer` changed from `500 9.5px` / `letterSpacing: 1.2` / halo `width: 2` to `400 8.5px` / `letterSpacing: 0.8` / halo `width: 1.5` — smaller, lighter weight, tighter, thinner halo than macrohood's unchanged `500 10px` / `1.4` / `2`. Measured against `public/basemap/zamboanga.pmtiles` (a one-off Node script using the `pmtiles` JS library's `FileSource` + `@mapbox/vector-tile`, decoding the actual tiles covering the archive's bounds at each zoom z11-z15, not the schema): at z13, in-view `places` features by `kind` were `neighbourhood: 187`, `locality: 147`, `macrohood: 32` — neighbourhood (subdivisions: "Purok Ⅳ", "Lo-ok", "Lipuno" among the names sampled) outnumbers macrohood (real barangays: "San Ramon", "Muruk", "Pasay", "Pitogo") nearly 6 to 1 at the exact zoom the old rule first drew both together. At z14/z15 the imbalance is similar (macrohood 37-38 vs neighbourhood 188-198), confirming this is a standing density difference, not a one-zoom blip a minzoom nudge alone would dodge. | The task offered three levers: push neighbourhood later, style it lighter, or honour the archive's own per-feature `min_zoom`. The third was checked and found to add nothing here: the cross-tab (`kind` × `min_zoom`) showed every in-view `neighbourhood` feature already carries `min_zoom: 13` — a single flat value, not a graduated one — and it already matched the old rule's `minzoom: 13` exactly; `macrohood` is similarly flat at `min_zoom: 11`. The archive doesn't grade importance within either kind in this cut, so "honouring min_zoom" is already what the code did, and it wasn't sufficient. The other two levers were both applied, not chosen either/or: pushing the tier later gives macrohood a full zoom (z11-z13) to read alone first; the lighter treatment keeps neighbourhood legible once it joins without matching macrohood's visual weight. `locality` (147 in-view features at z13, spanning `min_zoom` 6-13, rendered by the separate `label-settlement` rule with no `minzoom` floor of its own at all) was **measured but not touched** — it wasn't one of the reported defect's named examples (all of which were macrohood/neighbourhood pairs) and fixing it would be a second, unrequested tier change; flagged here for a future pass. |
| 2026-09-21 | **Defect 3 — the two curated DOM labels ("Mar de Basilán", "Bahía de Zamboanga") ignored the city mask and the canvas collision index.** `PlaceLabelsLayer`'s `placeLabels` pane moved from z-index 450 to 340 — still above the tile pane (200, Leaflet's own default) so it draws over the ground/roads, but now *below* `cityMask` (350) instead of above it. Folding the two names into `buildLabelRules` (one shared collision index, no second label system at all) was considered first and rejected: `buildLabelRules` only ever draws names the tile archive already carries, and the archive has no `water` feature for either name in view (re-confirmed, matching the original labels-reversal finding in this PRD) — there is no feature for a `LabelRule`'s `filter` to match. Moving the pane below the mask was the next-best option named in the task and the one taken: it closes the masking half of the inconsistency (a curated label is now hidden outside the city exactly like a canvas label already was), while the collision-index half stays exactly as this PRD already documented it (two entries, both over open water, low practical risk). `CityMask.mask-opacity.test.ts`'s pane-order test and `PlaceLabelsLayer.test.tsx`'s zIndex assertion were both flipped to the new, correct relationship rather than left asserting the old bug. | The two systems (DOM pane, canvas tile pane) were built years apart in this PRD's own timeline for different reasons — the curated pane predates the labels-reversal that made `buildLabelRules` the primary naming system — and never had their pane order re-checked against the mask's own fix (the mask-leak entry directly above this section verified the *old* 350-vs-450 order as correct without questioning whether 450-above-the-mask was itself right; it wasn't). |
