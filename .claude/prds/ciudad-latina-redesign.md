# PRD: Ciudad Latina redesign + Zamboanga-only map
...
Shipped in commits bd1a87b (bounds), 3b270cb (pins), ebc4fac (theme), fd64457 (UI + Chavacano copy), f282f4b (fix round: contrast, photo fallback, mobile header), b34d20d (azulejo tiling). 471 tests, tsc, lint, and build green. Reviewer approved after one rejection cycle. Chavacano strings in `src/lib/copy.ts` still need a native speaker's check.
